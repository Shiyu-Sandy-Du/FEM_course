...
def GLL_NodesAndWeights (n, epsilon = 1e-15):
  """
...
def GL_NodesAndWeights(n, epsilon=1e-15, maxiter=100):
    """
    ...
    epsilon : float
        Convergence tolerance for Halley's method.
    maxiter : int
    ...
    for i in range(n):
        ...
        for _ in range(maxiter):
            y  = lgP(n, np.array([xi]))
            y1 = dLgP(n, np.array([xi]))
            y2 = d2LgP(n, np.array([xi]))

            dx = 2.0 * y * y1 / (2.0 * y1**2 - y * y2)
            xi_new = xi - dx
            ...
            raise RuntimeError(f"Root iteration did not converge for i={i}")

        x[i] = xi
        w[i] = 2.0 / ((1.0 - xi**2) * y1**2)
...
def integrate_1d_using_gll_n(f, n_x):
    weights_x = GLL_NodesAndWeights(n_x)[1]
    # integral = 0.0
    # for i in range(n_x):
...
def integrate_2d_using_gll_n(f, n_x, n_y):
    weights_x = GLL_NodesAndWeights(n_x)[1]
    weights_y = GLL_NodesAndWeights(n_y)[1]
    integral = 0.0
    for i in range(n_x):
        for j in range(n_y):
...
def integrate_3d_using_gll_n(f, n_x, n_y, n_z):
    weights_x = GLL_NodesAndWeights(n_x)[1]
    weights_y = GLL_NodesAndWeights(n_y)[1]
    weights_z = GLL_NodesAndWeights(n_z)[1]
    integral = 0.0
    for i in range(n_x):
        for j in range(n_y):
            for k in range(n_z):
...
def interp_matrix_1D(ksi_N, x_hat_array):
    """
    Construct the 1D interpolation matrix from GLL points to arbitrary points.

    Parameters:
    ksi_N : array_like
        GLL nodes defined on [-1,1], length N+1
    x_hat_array : array_like
        Target interpolation points defined on [-1,1], shape (M,)

    Returns:
    interp_mat : ndarray
        Interpolation matrix of shape (M, N+1), where each row contains
        the interpolation weights for one target point.
    """
    N = len(ksi_N) - 1
    x_hat_array = np.asarray(x_hat_array)
    interp_mat = np.zeros((len(x_hat_array), N+1))
    for idx, x_hat in enumerate(x_hat_array):
        interp_mat[idx, :] = interp_1D_weights(ksi_N, N, x_hat)
    return interp_mat
