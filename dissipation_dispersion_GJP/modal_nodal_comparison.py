...
if __name__ == "__main__":
    ...
    kh_vals = np.linspace(1e-9, np.pi * p, npts)
    xplot = kh_vals / p

    lam_phys_nodal = physical_branch_Nodal_by_continuation(p, kh_vals, tau)
    lam_phys_modal = physical_branch_modal_by_continuation(p, kh_vals, tau)
    
    plt.figure(figsize=(4, 4))
    plt.plot(xplot, xplot, "--", color="gray")
    # for b in range(p):
    #     plt.plot(xplot, -branches[b].imag / p, color="tab:blue")
    plt.plot(xplot, -lam_phys_nodal.imag / p, color="tab:orange", linewidth=2, label="CG+GJP, nodal")
    plt.plot(xplot, -lam_phys_modal.imag / p, color="tab:blue", linewidth=2, label="CG+GJP, modal")
    plt.xlabel(r"$kh/P$")
    ...
    plt.tight_layout()
    plt.legend()
    plt.savefig("figures/dispersion_comparison.png", dpi=300)

    plt.figure(figsize=(4, 4))
    plt.plot(xplot, np.zeros_like(xplot), "--", color="gray")
    # for b in range(p):
    #     plt.plot(xplot, branches[b].real / p, color="tab:blue")
    plt.plot(xplot, lam_phys_nodal.real / p, color="tab:orange", linewidth=2, label="CG+GJP, nodal")
    plt.plot(xplot, lam_phys_modal.real / p, color="tab:blue", linewidth=2, label="CG+GJP, modal")
    plt.xlabel(r"$kh/P$")
    ...
    plt.tight_layout()
    plt.legend()
    plt.savefig("figures/dissipation_comparison.png", dpi=300)
    
    plt.figure(figsize=(4, 4))
    plt.plot(np.log10(xplot), np.log10(-lam_phys_nodal.real / p), color="tab:orange", linewidth=2, label="CG+GJP, nodal")
    plt.plot(np.log10(xplot), np.log10(-lam_phys_modal.real / p), color="tab:blue", linewidth=2, label="CG+GJP, modal")
    plt.xlabel(r"$\log_{10}(kh/P)$")
    plt.ylabel(r"$\log_{10}(-\mathrm{Re}(\lambda h)/P)$")
    plt.xlim(-2.5, 0.5)
    plt.ylim(-9.0, 0.5)
    plt.title(f"Inviscid CG dissipation, P={p}")
    plt.grid()
    plt.tight_layout()
    plt.legend()
    plt.savefig("figures/dissipation_comparison_loglog.png", dpi=300)
