...
def build_local_CG_matrices(p):
    p_raised = raised_order(p)
    n = p + 1
    n_raised = p_raised + 1
    xi, w = GLL_NodesAndWeights(n)
    dphi = GLL_DiffMatrix(xi)
    
    xi_raised, w_raised = GLL_NodesAndWeights(n_raised)
    dphi_raised = GLL_DiffMatrix(xi_raised)
...
def eigenvalues_CG(p, kh):
    return np.linalg.eigvals(build_Z_CG(p, kh))


def track_branches_CG(p, kh_vals):
    kh_vals = np.asarray(kh_vals)
    branches = np.zeros((p, len(kh_vals)), dtype=np.complex128)

    eigs0 = eigenvalues_CG(p, kh_vals[0])
    branches[:, 0] = eigs0[np.argsort(eigs0.imag)]

    for i in range(1, len(kh_vals)):
        eigs = list(eigenvalues_CG(p, kh_vals[i]))
...
if __name__ == "__main__":
    ...
    branches = track_branches_CG(p, kh_vals)
    lam_phys = physical_branch_CG(p, kh_vals)
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig("figures/dispersion.png", dpi=300)
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig("figures/dissipation.png", dpi=300)